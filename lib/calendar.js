...
try{
    const cal_events = [];
    const [cal_results] = await connection.query("Select start_time, end_time, schedule_id, c.name, DATE_FORMAT(date,'%Y-%m-%d') as date from doctor_calendar as d inner join clinics as c on c.clinic_id = d.clinic_id where doctor_id = ?", [user_id]);
    for(var i =0; i<cal_results.length; i++){
      ...
      cal_events.push({
        ...
        url: "/doctor/updateSchedule?scheduleId="+ cal_results[i]["schedule_id"]
      });
      cal_events.push({
        title: cal_results[i]["name"],
        start: cal_results[i]["date"],
      });
    }
